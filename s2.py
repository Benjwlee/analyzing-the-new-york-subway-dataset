...
#  return plot
  ...
  df_group = df.groupby(['Hour', 'rain'], as_index=False).sum()
  df_group.to_csv('dump.csv')
  plot = ggplot(df_group, aes(x='Hour', y='ENTRIESn_hourly', color='rain')) \
    + geom_line() \
    + ggtitle("Entries summation by hour of day, red=rain blue=no rain") \
    + xlab('Hour') + ylab('ENTRIESn') + scale_x_continuous()
...
if __name__ == "__main__":
    image = "plot.png"
    input_filename = 'C:/move - bwlee/Data Analysis/Nano/Intro to Data Science/\
    project/code/turnstile_data_master_with_weather.csv'
